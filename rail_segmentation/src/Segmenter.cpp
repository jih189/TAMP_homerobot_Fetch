...
Segmenter::Segmenter() : private_node_("~"), tf2_(tf_buffer_)
{
  ...
  search_table_srv_ = private_node_.advertiseService("search_table", &Segmenter::searchTableCallback, this);
...
bool Segmenter::searchTableCallback(rail_segmentation::SearchTable::Request &req,
        rail_segmentation::SearchTable::Response &res)
{
  ...
  if (zone.getRemoveSurface())
  {
    ...
    res.center = table_.center;
    res.centroid = table_.centroid;

    res.orientation = table_.orientation;
    res.width = table_.width;
