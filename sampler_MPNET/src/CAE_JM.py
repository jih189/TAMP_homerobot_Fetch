...
def main(args):	
	...
	for epoch in range(args.num_epochs):
		...
		avg_loss=0
		for i in range(0, len(obs)-100, args.batch_size):
			decoder.zero_grad()
			encoder.zero_grad()
		...
		# every 100 epochs, validate
		if epoch%100==0:
			avg_loss=0
			for i in range(len(obs)-100, len(obs), args.batch_size):
				inp = obs[i:i+args.batch_size]
				inp=torch.from_numpy(inp)
				inp =Variable(inp).cuda()
				# ===================forward=====================
			...
			if avg_loss/(100/args.batch_size)>=val_loss:
				# if validation loss is increasing, stop training
				break
			else:
				val_loss=avg_loss/(100/args.batch_size)
