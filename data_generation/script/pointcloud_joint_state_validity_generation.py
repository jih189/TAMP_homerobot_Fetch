...
class DataGenerator:
    ...
    def setObstaclesInScene(self, mesh_list):
        ...
        bad_obstacle_ids = self.checkInitCollision()
...
if __name__ == "__main__":
    ...
    rospy.init_node('pointcloud_joint_state_validity_generation_node', anonymous=True)

    # get parameters
    random_value = rospy.get_param('~random_value', 0)
    total_env_num = rospy.get_param('~total_env_num', 100)

    data_generator = DataGenerator(moveit_commander)
    ...
    print " number of valid robot states: ", valid_robot_states.shape[0]
    print " random value: ", random_value
    print " total number of environments: ", total_env_num

    for i in tqdm(range(random_value, random_value+total_env_num)):

        env_num = i

        obstacle_meshes = data_generator.generate_random_mesh(env_num)
        pointcloud = data_generator.setObstaclesInScene(obstacle_meshes)

        # create a dir with the env_num
        if os.path.exists(package_path + "/gmm_data/env_%06d" % env_num):
            # remove the dir
            os.system("rm -rf " + package_path + "/gmm_data/env_%06d" % env_num)
        os.makedirs(package_path + "/gmm_data/env_%06d" % env_num)

        write_ply(package_path + "/gmm_data/env_%06d/map_" % env_num + "%d.ply" % env_num, pointcloud)
        ...
        np.save(package_path + "/gmm_data/env_%06d/valid_tag.npy" % env_num, valid_tag)

        data_generator.cleanPlanningScene()