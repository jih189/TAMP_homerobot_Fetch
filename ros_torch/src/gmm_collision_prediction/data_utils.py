...
class GMMDataset(data.Dataset):
    ...
    def __init__(self, dir_path, split_list, gmm_path = None, ):
        ...
        # self.zero_loss_masks = np.ones((self.num_distributions))
        zero_loss_masks = []
        ...
        self.zero_loss_masks = np.array(zero_loss_masks) * 0.1
    ...
    def __getitem__(self, index):
        ...
        valid_configs_labels = np.load("table_valid.npy")
        ...
        for dist_no, arm_config_gmm_index in enumerate(self.arm_config_gmm_indices):
            ...
            if len(gmm_valid_val):
                labels.append((gmm_valid_val.sum() / len(gmm_valid_val)).squeeze())
            else :
                labels.append(0.0)
        ...
        dpone
        return pc, self.zero_loss_masks, np.array(labels).astype(np.float32)
...
def create_dataloader():
    ...
    generated_data_dir = "/src/gmm_data/"
    ...
    # val_list = train_list[::5]
    print(len(train_list), len(test_list), len(val_list))

    train_dataset = GMMDataset(dir_path=generated_data_dir, split_list=train_list, gmm_path="/tmp/dpgmm/")
    ...
    val_dataset = GMMDataset(dir_path=generated_data_dir, split_list=val_list, gmm_path="/tmp/dpgmm/")
