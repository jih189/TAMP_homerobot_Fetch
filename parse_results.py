...
with open('experiments_4.log') as f:
    content = f.readlines()
    for line in content:
        if "Scene: " in line:
        ...
        if "Difference in rotation" in line:
            ...
            if rotation_text == "None":
                ...
                rotation = float(rotation_text)*2
...
for object in raw_data['target_object'].unique():
    for planner_type in raw_data['planner_type'].unique():
        ...
        data = data.append({'object': object, 'planner_type': planner_type, 'success_rate': success_rate, 'average_stability': average_stability}, ignore_index=True)

# print the data and write it to a latex table
print(data)
data.to_latex('experiments_4.tex', index=False)
