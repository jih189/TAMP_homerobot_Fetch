...
class DynamicMTGTaskPlannerWithAtlas(BaseTaskPlanner):
    def __init__(self, gmm, default_robot_state, planner_name_="DynamicMTGTaskPlannerWithAtlas", parameter_dict_={}):
        # Constructor
        super(BaseTaskPlanner, self).__init__()
        # super().__init__() # python 3
    ...
    def filter_node(self, node):
        ...
        return self.task_graph.nodes[node]['dist_to_start'] + self.task_graph.nodes[node]['dist_to_goal'] <= distance
    ...
    def expand_current_task_graph(self, distance):
        ...
        # self.current_task_graph = nx.subgraph_view(self.task_graph, filter_node=self.filter_node)        
        print("expand_current_task_graph takes %f seconds"%(time.time() - time_start))
    ...
    def set_start_and_goal(self,
        ...
        print(self.current_graph_distance_radius)
    ...
    def generate_task_sequence(self):
        ...
        print("Searching the graph which has %d nodes with radius %f"%(len(self.current_task_graph.nodes()), self.current_graph_distance_radius))
        # check the connectivity of the task graph from start to goal
    ...
    def update(self, task_graph_info_, plan_, manifold_constraint_):
        ...
        self.current_graph_distance_radius *= 1.1
        self.expand_current_task_graph(self.current_graph_distance_radius)
