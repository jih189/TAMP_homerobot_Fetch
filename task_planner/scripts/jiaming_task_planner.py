...
class MTGTaskPlannerWithAtlas(BaseTaskPlanner):
    ...
    def update(self, task_graph_info_, plan_, manifold_constraint_):
        ...
        for distribution_index in range(len(self.gmm_.distributions)):
            ...
            invalid_configuration_number_before_project = sampled_data_distribution_tag_table[distribution_index][5] + \
                                                          sampled_data_distribution_tag_table[distribution_index][6] + \
                                                          sampled_data_distribution_tag_table[distribution_index][7]
            self.task_graph.nodes[(current_manifold_id[0], current_manifold_id[1], distribution_index)][
                'invalid_configuration_before_project'] += invalid_configuration_number_before_project
            # if there are some projected valid configuration, then there must be an atlas.
        ...
        for n in self.task_graph.nodes():
            ...
            self.task_graph.nodes[(current_manifold_id[0], current_manifold_id[1], node_gmm_id)][
                'invalid_configuration_before_project']
            related_node_valid_configuration_before_project = \
            self.task_graph.nodes[(current_manifold_id[0], current_manifold_id[1], node_gmm_id)][
                'valid_configuration_before_project']
            has_atlas = self.task_graph.nodes[(current_manifold_id[0], current_manifold_id[1], node_gmm_id)][
                'has_atlas']
            ...
            if not has_atlas:
                if (
                        related_node_invalid_configuration_before_project + related_node_valid_configuration_before_project == 0):
                ...
                if (
                    ...
                    beta_value = 1.0 * related_node_invalid_configuration_before_project / (
                                related_node_invalid_configuration_before_project + related_node_valid_configuration_before_project)

            arm_env_collision_score_after_project = sampled_data_distribution_tag_table[node_gmm_id][1] * 1.0
            path_constraint_violation_score_after_project = current_similarity_score * \
                                                            sampled_data_distribution_tag_table[node_gmm_id][2] * 1.0
            obj_env_collision_score_after_project = current_similarity_score * \
                                                    sampled_data_distribution_tag_table[node_gmm_id][3] * 1.0
            success_score_after_project = current_similarity_score * sampled_data_distribution_tag_table[node_gmm_id][
                0] * 0.01

            arm_env_collision_score_before_project = sampled_data_distribution_tag_table[node_gmm_id][5] * 1.0
            path_constraint_violation_score_before_project = current_similarity_score * \
                                                             sampled_data_distribution_tag_table[node_gmm_id][6] * 1.0
            obj_env_collision_score_before_project = current_similarity_score * \
                                                     sampled_data_distribution_tag_table[node_gmm_id][7] * 1.0
            success_score_before_project = current_similarity_score * sampled_data_distribution_tag_table[node_gmm_id][
                4] * 0.01

            self.task_graph.nodes[n]['weight'] += (
                    (1 - beta_value) * (
                        success_score_after_project + arm_env_collision_score_before_project + path_constraint_violation_score_before_project + obj_env_collision_score_before_project) +
                    beta_value * (
                                success_score_before_project + arm_env_collision_score_after_project + path_constraint_violation_score_after_project + obj_env_collision_score_after_project)
            )
